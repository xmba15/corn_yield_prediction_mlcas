...
from src.models import CornYieldCNN
from src.utils import fix_seed


def get_args():
    parser = argparse.ArgumentParser("train cnn lstm")
    parser.add_argument("--config_path", type=str, default="./config/base_cnn.yaml")
...
def main():
    ...
    use_pseudo_labeling = "use_pseudo_labeling" in hparams["dataset"] and hparams["dataset"]["use_pseudo_labeling"]

    if use_pseudo_labeling:
        print("train with pseudo labeling")
        val_pseudo_dataset = CornYieldDataset(
        ...
        test_pseudo_dataset = CornYieldDataset(
    ...
    if "pretrained_model_weights" in hparams["trainer"] and os.path.isfile(
        hparams["trainer"]["pretrained_model_weights"]
    ):
